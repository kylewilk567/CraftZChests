...
public class CustomLootTable1_12 {
	...
	public Collection<ItemStack> populateLoot(Random random, String tier) {
		...
		config.getConfigurationSection("items").getKeys(false).forEach(key -> {
			...
			if(config.getString("items." + key + ".material").equalsIgnoreCase("INK_SACK")) {
				Dye dye = new Dye();
				dye.setColor(DyeColor.valueOf(config.getString("items." + key + ".dye_color")));
				item = dye.toItemStack(config.getInt("items." + key + ".amount"));
			}
			else {
			...
			if(config.getDouble("items." + key + ".max_durability") != 0 && config.getDouble("items." + key + ".min_durability") != 0) {
				item.setDurability(this.getDurability(key, tier, item.getType()));
			}
